...
假设代理主机系统已经完成启动，已订阅基础操作系统通道（如 {sles} 15 SP7 或 {sl-micro} 6.2）以及代理扩展通道。
...
*** 对于 `Registry URL`，请使用：`registry.suse.com/suse/multi-linux-manager/5.2/x86_64`。

*** 从下拉列表中选择标记。

** 如果选择 `Advanced`，表单会显示一个额外的部分：

*** 对于每个单独的容器 URL 字段，请使用注册表：`registry.suse.com/suse/multi-linux-manager/5.2/x86_64` 后跟相应的后缀，例如 `_proxy-httpd_` 或 `_salt-broker_`。
