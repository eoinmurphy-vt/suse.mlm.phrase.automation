...
*virt-manager*를 사용하여 새 가상 머신을 생성할 때 다음 설정을 입력하십시오.
...
| 메모리:             | 최소 *)
| CPU:              | 최소 *)
| 스토리지 형식:     | _.qcow2_ 40 GB (기본) 루트 파티션
| 이름:               | 테스트 설정
| 네트워크             | 브리지 _br0_
|===
...
.절차 초기 설정 생성 중
. 다운로드한 {minimal} KVM 이미지를 사용하여 새 가상 머신을 생성하고 [guimenu]``+기존 디스크 이미지 가져오기+``를 선택합니다.
. RAM 및 CPU 수를 구성합니다.
. KVM 머신의 이름을 지정합니다.
