...
. btn:[Create]를 클릭하고 드롭다운 메뉴에서 [guimenu]``+Azure+``를 선택합니다.
...
* [guimenu]``+영역+``+ 필드에 VM이 있는 영역을 입력합니다. 예를 들어, 서유럽의 경우 [path]+``+westeurope+``를 입력합니다.
    이 키는 작업과 일치하는 등록을 위해 필요합니다.
. btn:[Create]를 클릭하여 변경 사항을 저장하고 VHM을 생성합니다.
. [guimenu]``+가상 호스트 관리자+`` 페이지에서 새 VHM을 선택합니다.
. [guimenu]``+속성+`` 페이지에서 btn:[데이터 새로 고침]을 클릭하여 새 VHM이 목록에 반영되도록 합니다.
