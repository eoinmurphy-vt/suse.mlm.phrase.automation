...
[NOTE]
====
이 옵션은 [command]``+virtual-host-gatherer+`` 도구로 생성된 파일을 가져오는 데 사용됩니다.
...
. VM 네트워크에서 [command]``+virtual-host-gatherer+``를 실행하여 JSON 구성 파일을 내보냅니다.
...
* [guimenu]``+Url+`` 필드에 내보낸 JSON 구성 파일의 경로를 입력합니다.
. btn:[생성]을 클릭하여 변경 사항을 저장하고 VHM을 생성합니다.
. [guimenu]``+가상 호스트 관리자+`` 페이지에서 새 VHM을 선택합니다.
. [guimenu]``+속성+`` 페이지에서 btn:[데이터 새로 고침]을 클릭하여 새 VHM을 목록에 추가합니다.
...
자세한 내용은 {productname} 서버의 man 페이지에서 [command]``+virtual-host-gatherer+``를 참조하십시오:
