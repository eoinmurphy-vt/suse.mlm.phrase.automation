...
{cobbler} [command]``+buildiso+`` 명령은 부팅 ISO의 이름 및 출력 위치를 정의하는 파라미터를 받아들입니다.
[command]+``+buildiso+`` 명령을 실행할 때 [option]``+--distro+``+로 배포를 지정하는 것은 필수입니다.
[option]``+--iso+``는 출력 위치입니다:
...
[option]``+--profiles+``+ 및 [option]+``+--systems+`` 옵션을 사용하여 사용되는 프로파일과 시스템을 제한할 수 있습니다:
...
[option]``+--esp+``를 사용하면 보안 부팅이 있는 부팅 ISO를 활성화할 수 있습니다.
...
. 컨테이너 호스트에서 다음 명령을 실행하여 [option]``+distro+`` 파일의 위치에 대한 정보를 가져옵니다:
+
----
...
. 부팅 디렉토리의 내용을 확인하십시오.
  위의 예제에서, 부팅 파티션은 [path]``+/usr/share/tftpboot-installation/SLE-15-SP6-x86_64/boot/x86_64/efi+`` 파일이지만, 이 파티션은 ISO 배포자에 따라 다를 수 있습니다.
...
ISO 이미지를 사용하려면 시스템을 부팅하고 프롬프트에 `+autoyast+`을 입력하십시오(부팅 레이블을 {ay} 부팅으로 ``+autoyast+``로 남겨두었다고 가정).
