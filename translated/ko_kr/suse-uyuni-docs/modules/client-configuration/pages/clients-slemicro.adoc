...
{productname}에 의해 관리되는 시스템에서 재부팅할 때는 [systemitem]``+systemd+``를 사용하는 것이 권장됩니다.
다른 방법을 사용하면 원치 않는 동작이 발생할 수 있습니다.

{productname}에서 트랜잭션 시스템을 부팅할 때, 시스템이 기본 구성에 있는 경우 [systemitem]``+systemd+``+가 재부팅 방법으로 구성됩니다 ([literal]+``\+REBOOT_METHOD+</p-3]).
이러한 구성은 {productname}가 재부팅 작업을 제어할 수 있게 하며, 재부팅은 즉시 수행되거나 원할 경우 {productname}로 예약할 수 있습니다.


=== 배경 정보

기본적으로 클라이언트 설치 중 재부팅 방법은 [option]``+auto+``로 설정됩니다.
[option]``+auto+``+ 부팅 방법을 사용하면, 서비스가 실행 중인 경우 [command]+``+rebootmgrd+``가 구성된 정책에 따라 시스템을 재부팅하는 데 사용됩니다.
정책은 즉시 재부팅하거나 유지 관리 창 동안 재부팅할 수 있습니다.
자세한 내용은 [command]``+rebootmgrd+``(8) man 페이지를 참조하십시오.
만약 [command]``+rebootmgrd+``+가 실행 중이 아니면, {productname}는 [command]+``+systemctl reboot+``를 호출합니다.

[NOTE]
====
[command]``+systemd+``와 다른 방법을 사용하면 원치 않는 동작이 발생할 수 있습니다.
====
