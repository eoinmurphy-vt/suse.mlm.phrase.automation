...
.절차 클라이언트 업그레이드 준비
. {productname} 서버의 명령 프롬프트에서 루트 사용자로 로그인한 후, [command]``+spacewalk-common-channels+`` 명령을 사용하여 적절한 채널을 추가합니다.
+
----
...
. {productname} 서버 {webui}에서 menu:소프트웨어[채널 > 관리]으로 이동하여 [systemitem]``+Uyuni Client Tools for openSUSE Leap 15.4 (x86_64)+`` 채널 이름을 클릭합니다.

. 오른쪽 상단에서 btn:[채널 관리]를 클릭합니다.

. [guimenu]``+리포지토리+`` 탭을 클릭하고 [systemitem]+``+External - Uyuni Client Tools for openSUSE Leap 15.3 (x86_64)+``를 선택합니다.

. btn:[리포지토리 업데이트]를 클릭합니다.

. menu:리포지토리[동기화] 하위 탭으로 이동하여 btn:[지금 동기화]를 클릭합니다.

. [systemitem]``+openSUSE Leap 15.4 (x86_64)+``+ 및 [systemitem]+``+External - openSUSE Leap 15.3 (x86_64)+``에 대해서도 동일한 작업을 합니다.
...
. menu:소프트웨어[소프트웨어 채널]을 클릭하고 기본 채널로 {opensuse-version} 채널을 선택합니다. 이 채널은 [systemitem]``+사용자 정의 채널+`` 목록에 나열되어 있습니다.
. [guimenu]``+하위 채널+`` 창에서 {opensuse-version} 하위 채널을 선택합니다.
. btn:[다음]을 클릭하고 [guimenu]``+소프트웨어 채널 변경 확인+``을 btn:[확인]합니다.
