...
[NOTE]
====
增强的 SCAP 审计体验作为测试版功能提供。
它提供集中式内容管理、可重用的扫描策略、定期扫描以及来自 {webui} 的内置修复。
要开始使用，请参阅 xref:administration:openscap-enhanced-beta.adoc[增强的 SCAP 审计（测试版）]。
====
...
确保每个要扫描的客户端上至少有 2{nbsp}GB 的可用 RAM（特别是对于 {redhat} 及其他现代客户端）。
====
...
| SUSE Liberty footnote:[SUSE Liberty Linux 包含一个兼容的 `+scap-security-guide+` 软件包。但是，随客户端工具一起提供的版本不兼容，因此不应将其用于 SCAP 内容。]| 7、8、9、10 | `+openscap-utils+` | `+scap-security-guide+`
| Debian | 13 | `+openscap-scanner+` | 不受官方支持 footnote:[此软件包由社区提供，不受 {suse} 的官方支持。上游 `+ssg-debian+` 可供使用，但风险自负。]
| Ubuntu | 22.04 | `+libopenscap8+` | `+scap-security-guide-ubuntu+`
| Ubuntu | 24.04 | `+openscap-scanner+` | 不受官方支持 footnote:[此软件包由社区提供，不受 {suse} 的官方支持。上游 `+ssg-debderived+` 可供使用，但风险自负。]
|===

[NOTE]
====
对于 Debian 13 和 Ubuntu 24.04，{suse} 不提供自定义安全指南软件包。
要在这些客户端上执行扫描，您必须从其各自的社区存储库中安装上游软件包（分别为 `+ssg-debian+` 和 `+ssg-debderived+`）。
请注意，这些上游指南不受 {suse} 的官方支持。
====
...
[NOTE]
====
如果您使用 xref:administration:openscap-enhanced-beta.adoc[增强的 SCAP 审计（测试版）] 功能，则无需手动传输内容文件。服务器会在运行时自动将所需的 DataStreams 和定制文件部署到客户端。
====
...
[TIP]
====
如果您启用了测试版 SCAP 功能，则可以直接从 {webui} 中的扫描结果应用修复，包括针对每个规则的自定义修复脚本。
请参阅 xref:administration:openscap-enhanced-beta.adoc#remediation-enhanced[增强的 SCAP 审计 - 修复]。
====
