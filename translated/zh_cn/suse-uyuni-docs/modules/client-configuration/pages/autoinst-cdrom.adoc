...
{cobbler} [command]``+buildiso+`` 命令接受用于定义引导 ISO 的名称和输出位置的参数。
运行 [command]+``+buildiso+`` 命令时，必须使用 [option]``+--distro+``+ 指定发行套件。[option]``+--iso+`` 是输出位置：
...
您可以使用 [option]``+--控制文件+``+ 和 [option]+``+--系统+`` 选项限制使用的控制文件和系统：
...
使用 [option]``+--esp+``，您可以启用带有安全启动的构建引导 ISO。
您明确指定 EFI 系统分区 (ESP)。
...
. 查看引导目录的内容。
  在上例中，引导分区为 [path]``+/usr/share/tftpboot-installation/SLE-15-SP6-x86_64/boot/x86_64/efi+`` 文件，但这可能会因 ISO 发行商而异。



[NOTE]
====
如果您无法将 ISO 映像写入公共 [path]``+tmp+``+ 目录，请检查 [path]+``+/usr/lib/systemd/system/cobblerd.service+`` 中的 systemd 设置。
====
