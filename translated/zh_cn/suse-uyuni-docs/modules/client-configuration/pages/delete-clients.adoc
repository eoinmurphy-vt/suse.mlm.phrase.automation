[[delete.clients]]
= 客户端删除
:description: 通过 Web UI 或命令行选项在您的 SUSE Multi-Linux Manager 服务器中配置客户端
:revdate: 2025-04-28
:page-revdate: {revdate}
:page-author: SUSE 产品与解决方案文档团队
:page-image: https://www.suse.com/assets/img/suse-white-logo-green.svg
:page-product-name: SUSE Multi-Linux Manager
:page-product-version: 5.1

// FIXME: where do we need to add warnings (suse clients only, all clients)

如果需要从您的 {productname} 服务器中去除客户端，您可以：

* 使用 {webui} 删除它
* 从命令行去除客户端。

// can also be done manually.
// FIXME: Why Manual Cleanup is necessary sometimes.



[[delete.clients.webui]]
== 使用 {webui} 删除客户端


.过程：升级删除客户端
[role=procedure]
____
. 在 {productname} {webui} 中，导航到 menu:系统[系统列表] 并选择要删除的客户端。

. 点击 btn:[删除系统]。

. 检查细节后，点击 btn:[删除控制文件] 以确认。

. 对于 {salt} 客户端，{productname} 尝试清理额外的配置。
  如果无法联系到客户端，您可以选择取消删除，或在不清理配置文件的情况下删除客户端。

____

您还可以使用系统集管理器删除多个客户端。
有关系统集管理器的更多信息，请参见 xref:client-configuration:system-set-manager.adoc[]。

[IMPORTANT]
====
清理客户端仅禁用 {salt}，并在可能的情况下停止服务。
它不会卸载软件包。
====



== 在命令行上删除客户端（通过 API 调用）

.过程：升级从服务器上删除客户端
[role=procedure]
____
. 使用 FQDN（完全限定的域名）删除客户端：

+

----
spacecmd system_delete FQDN
----
____

[command]``+spacecmd system_delete+`` 还会删除 {salt} 密钥。

[command]``+system_delete+`` 提供以下选项：


----
usage: system_delete [options] <SYSTEMS>

    options:
          -c TYPE - Possible values:
             *  'FAIL_ON_CLEANUP_ERR' - fail in case of cleanup error,
             *  'NO_CLEANUP' - do not cleanup, just delete,
             *  'FORCE_DELETE' - try cleanup first but delete server anyway
            in case of error
----

////
// move to Trouble Shooting and link from here
Sometimes a new registration of a deleted (unregistered) client might not be possible.
To solve this issue, some Salt cache files should be deleted on the {productname} Server (Salt master) before trying to re-register again:

----
rm /var/cache/salt/master/thin/version
rm /var/cache/salt/master/thin/thin.tgz
----
////



[[delete.clients.commandline]]
== 从命令行删除客户端

[IMPORTANT]
====
此过程仅适用于 {productname} 客户端。
请勿在 {productname} 服务器本身上运行。
====

[WARNING]
====
您不得在运行 {rhel}、{debian} 或克隆的客户端上执行以下程序。
请使用等效的打包命令，例如 [command]+``+yum+``+、[command]+``+dnf+``+ 或 [command]+``+apt+``，而不是 [command]``+zypper+``+。
====

.过程：升级删除 SLES 12 和 15 客户端
[role=procedure]
____
. 停止 salt-minion 服务：

+

[source,shell]
----
systemctl stop salt-minion
----

+

. 去除储存库和配置文件：

+

[source,shell]
----
rm -f /etc/zypp/repos.d/susemanager\:bootstrap.repo
rm -f /etc/zypp/repos.d/susemanager\:channels.repo
rm -r /etc/sysconfig/rhn/
rm -r /etc/salt/
----

+

. 去除客户端软件包：

+

[source,shell]
----
zypper rm salt salt-minion python*-salt sle-manager-tools-release
----

____


.过程：{salt} 捆绑包客户端 - 手动进行注册清理
[role=procedure]
____
. 要取消注册，请运行以下命令：

+

[source,shell]
----
systemctl stop venv-salt-minion
zypper rm -y venv-salt-minion sle-manager-tools-release
rm -f /etc/zypp/repos.d/susemanager\:bootstrap.repo
rm -f /etc/zypp/repos.d/susemanager\:channels.repo
rm -r /etc/venv-salt-minion/*
----

. 去除客户端计算机 ID：

+

----
rm /etc/sysconfig/rhn/systemid
----

____

有关 {salt} 捆绑包的信息，请参见 xref:client-configuration:contact-methods-saltbundle.adoc[]。
