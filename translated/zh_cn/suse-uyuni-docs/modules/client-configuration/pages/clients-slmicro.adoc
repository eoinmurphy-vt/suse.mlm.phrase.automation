...
{sl-micro} 是一个事务型系统。
事务更新通常支持多种重引导方法。
建议在由 {productname} 管理的系统中使用 [systemitem]``+systemd+`` 进行重引导。
使用其他方法可能会导致不希望的行为。

在 {productname} 上启动事务型系统时，如果系统处于默认配置，[systemitem]``+systemd+``+ 将被配置为重引导方法（[literal]+``+REBOOT_METHOD+``）。
这样的配置允许 {productname} 控制重引导操作，重引导可以立即执行或根据需要与 {productname} 安排。


=== 背景信息

默认情况下，客户端安装期间的重引导方法设置为 [option]``+auto+``。
使用 [option]``+auto+``+ 启动方法时，如果服务正在运行，[command]+``+rebootmgrd+`` 将根据配置的策略重引导系统。
...
[NOTE]
====
使用任何不同于 [command]``+systemd+`` 的方法可能会导致不希望的行为。
====
