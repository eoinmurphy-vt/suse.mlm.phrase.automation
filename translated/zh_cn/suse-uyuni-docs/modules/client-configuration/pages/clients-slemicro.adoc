...
它们不影响正在运行的系统，因为只有在重启系统后，才会激活更改。
...
此窗口可能在客户端注册后几个小时。
建议在注册脚本完成后手动重启客户端，以加快注册速度并使系统出现在系统列表中。
====
...
{sle-micro} 是一个事务型系统。
事务更新通常支持几种重启方法。
建议在由 {productname} 管理的系统中使用 [systemitem]``+systemd+`` 进行重启。
使用其他方法可能会导致不期望的行为。

在 {productname} 上启动事务型系统时，如果系统处于默认配置，[systemitem]``+systemd+``+ 将被配置为重启方法（[literal]+``+REBOOT_METHOD+``）。
这样的配置允许 {productname} 控制重启操作，重启可以立即执行或根据需要与 {productname} 进行调度。


=== 背景信息

默认情况下，客户端安装期间的重启方法设置为 [option]``+auto+``。
使用 [option]``+auto+``+ 启动方法时，如果服务正在运行，将使用 [command]+``+rebootmgrd+`` 根据配置的策略重启系统。
策略可以是立即重启或在维护窗口期间重启。
有关详细信息，请参见 [command]``+rebootmgrd+``(8) 手册页。
否则，如果 [command]``+rebootmgrd+``+ 未在运行，{productname} 将调用 [command]+``+systemctl reboot+``。

[NOTE]
====
使用与 [command]``+systemd+`` 不同的任何方法可能会导致不期望的行为。
====
