...
该过程包括安装 [command]``+Podman+``+、[systemitem]+``+Uyuni 容器工具+``+、部署，然后通过 [command]+``+mgrctl+`` 开始与容器的交互。
...
该步骤可选。
但是，如果您的基础架构需要自定义的永久性存储，请使用 [command]``+mgr-storage-server+`` 工具。
...
{productname} 使用 [command]``+mgradm+`` 工具作为容器进行部署。
...
要获取更多信息，您可以在命令行中运行 [command]``+mgradm --help+`` 进行进一步探索。
...
您可以通过在此路径或其中预期的卷路径上挂载磁盘来提供自定义存储，例如：[path]``+/var/lib/containers/storage/volumes/var-spacewalk+``。
这对于数据库和软件包镜像尤其重要。
