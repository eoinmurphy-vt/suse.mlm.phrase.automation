...
* 为 {productname} 服务器签名的证书颁发机构 (CA) 的公共证书。
* 为代理创建激活密钥
* 附加到代理证书的其他信息。
...
如果不使用第三方证书，可以在 `+rhn-ssl-tool+` 容器内使用 {productname} 生成证书。
...
* 如果可连接容器映像仓库，则可使用[literal]``+仓库+``选项。 +
如果选择此选项，系统会提示您从两个子选项中选择：[literal]``+简单+``或 [literal]+``+高级+``。
+
** 如果选择[literal]``+简单+``，需要在[literal]+``+仓库 URL+``+ 和 [literal]+``+容器标记+`` 字段中输入对应值。
*** 在[literal]``+仓库 URL+``中使用： [literal]+``+registry.opensuse.org/uyuni+``。
*** 从下拉列表中选择标记。

** 如果选择[literal]``+高级+``，表单会显示一个额外的部分：
*** 对于每个独立容器的 URL 字段，使用仓库：[literal]``+registry.opensuse.org/uyuni+``，后面跟上相应的后缀，例如 +`_proxy-httpd_`+ 或 +`_salt-broker_`。
*** 从下拉列表中选择标记。

. 填写完所有字段后，单击 btn:[应用] 以应用配置并调度代理安装任务。
