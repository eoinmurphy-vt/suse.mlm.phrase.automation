...
有关{suse}安全团队的更多信息，请参见https://www.suse.com/support/security.。
...
RHEL 7 及兼容系统提供了一个 [package]``+scap-security-guide+`` 包，其中包含过时的内容。
建议您使用在 {productname} 客户端工具中找到的 [package]``+scap-security-guide-redhat+`` 包。


[IMPORTANT]
====
{suse} 提供了不同 openscap 控制文件的 [package]``+scap-security-guide+`` 包。
在当前版本的 [package]``+scap-security-guide+`` 中，{suse} 支持以下控制文件：
...
您可以像移动其他文件一样进行此操作，使用物理存储介质，或通过网络使用 Salt（例如，https://docs.saltproject.io/en/latest/ref/cli/salt-cp.html[salt-cp] 或 https://docs.saltproject.io/en/latest/ref/file_server/index.html[Salt 文件服务器]），[command]``+FTP+``+ 或 [command]+``+scp+``。
...
. 导航到 [guimenu]``+审计+``+ 选项卡和 [guimenu]+``+日程安排+`` 子选项卡。

. 在 [guimenu]``+XCCDF 文档的路径+`` 字段中，输入要在客户端上使用的 SCAP 模板和控制文件的参数。
  例如：
...
[NOTE]
====
如果您经常使用 [literal]``+--fetch-remote-resources+`` 参数，则需要大量 RAM。
此外，您可能需要增加 [literal]``+file_recv_max_size+`` 的值。
====
...
* ``+<path_to_xccdf_file.xml>+`` 是客户端上内容文件位置的路径。
  例如，[path]``+/usr/share/xml/scap/ssg/content/ssg-sle15-ds.xml+``。

* ``+<profile_name>+``+ 是 [command]+``+oscap+`` 命令的附加参数。
...
导航到 [guimenu]``+配置+``+ 选项卡，并检查 [guimenu]+``+启用详细 SCAP 文件上传+`` 选项。
...
您可以在 [literal]``+Organization Configuration+`` 部分配置保留期。
...
. 在 [guimenu]``+SCAP+``+ 下，勾选 [guimenu]+``+允许删除 SCAP 结果+``。

. 在 [guimenu]``+允许删除后+`` 中输入天数（默认值为 90）。
...
在所有目标系统上安装 [package]``+scap-security-guide+`` 软件包。
...
. 创建系统组后，单击 [guimenu]``+系统组+``+，从表中选择 [guimenu]+``+在 SSM 中使用+``。

. 在 [guimenu]``+系统集管理器+`` 中，在 menu:其他[远程命令] 下，编写一个 Bash 脚本，例如：
+
