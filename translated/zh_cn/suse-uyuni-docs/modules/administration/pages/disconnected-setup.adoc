...
您可以在 `+cli+` 部分的 [path]``+/etc/rmt.conf+`` 中更改 RMT 用户设置。
====
...
====
确保外部存储已挂载到 RMT 用户可写的目录。
您可以在 [path]``+/etc/smt.conf+`` 中更改 SMT 用户设置。
====
...
请注意，执行最后一个命令时要包含 [option]``+--mirror+``+ 选项，并将 [literal]+``+</media/disk>+`` 替换为您的安装点：
...
[NOTE]
====
请注意，如果 [literal]``+server.susemanager.fromdir+`` 被设置，{productname} 将无法检查 {scclongform} 的凭据是否有效。
