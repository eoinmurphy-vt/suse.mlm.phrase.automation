...
还可以检查 Zypper 在 [path]``+/var/log/zypper.log+`` 创建的日志。


GPG 密钥不匹配::
{productname} 不会自动信任第三方 GPG 密钥。
如果软件包同步失败，可能是由于不受信任的 GPG 密钥。
您可以通过打开 [path]``+/var/log/rhn/reposync+`` 并查找如下错误来确定是否是这种情况：
+
----
...
从 [literal]``+spacewalk-repo-sync+`` 中去除 GPG 密钥::
使用 [literal]``+spacewalk-repo-sync+``+ 手动导入储存库的 GPG 密钥后，如果此密钥不再需要（例如，当该密钥被泄露或者仅用于测试目的时），可以通过以下命令从 [literal]+``+spacewalk-repo-sync+`` 使用的 zypper RPM 数据库中去除该密钥：
+
----
...
----
+
可以通过在 [path]+``+/etc/rhn/rhn.conf+`` 中指定 [literal]``+reposync_timeout+``+ 和 [literal]+``+reposync_minrate+``+ 配置值来解决此错误。
默认情况下，当在 300 秒内传输的字节数少于 1000 字节时，下载将被中止。
您可以通过 [literal]``+reposync_minrate+``+ 调整每秒传输的字节数，并通过 [literal]+``+reposync_timeout+`` 调整等待的秒数。


重新同步期间手动信任密钥::
在某些情况下，当运行 [command]``+reposync+`` 时，您可能需要手动接受 GPG 密钥。
例如：
+
----
