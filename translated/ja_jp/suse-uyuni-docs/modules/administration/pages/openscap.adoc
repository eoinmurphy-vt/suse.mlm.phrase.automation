...
RHEL 7および互換性のあるシステムは、古い内容を含む[package]``+scap-security-guide+`` パッケージを提供します。
[package]``+scap-security-guide-redhat+``パッケージはクライアントツール内の{productname}で見つかりますので、そちらを使用することをお勧めします。


[IMPORTANT]
====
{suse}は、さまざまなopenscapプロファイル用の[package]``+scap-security-guide+`` パッケージを提供します。
現在の[package]``+scap-security-guide+``のバージョンでは、{suse}は次のプロファイルをサポートしています：
...
これは、物理ストレージメディアを使用するか、Saltを介してネットワーク上で他のファイルを移動するのと同じ方法で行うことができます（例えば、https://docs.saltproject.io/en/latest/ref/cli/salt-cp.html[salt-cp]やhttps://docs.saltproject.io/en/latest/ref/file_server/index.html[Saltファイルサーバ]、[command]``+ftp+``+または[command]+``+scp+``）。
...
. [guimenu]``+監査+``+タブと、[guimenu]+``+スケジュール+``サブタブに移動します。

. [guimenu]``+XCCDFドキュメントへのパス+``フィールドに、クライアントで使用するSCAPテンプレートとプロファイルのパラメータを入力します。
  例:

+

  * [literal]``+コマンド+``: /usr/bin/oscap xccdf eval

  * [literal]``+Command-line arguments+``: --profile xccdf_org.ssgproject.content_profile_stig

  * [literal]``+XCCDFドキュメントへのパス+``: /usr/share/xml/scap/ssg/content/ssg-sle15-ds.xml

+

[NOTE]
====
[literal]``+--リモートリソースを取得する+``パラメータを使用すると、大量のRAMが必要です。
さらに、[literal]``+file_recv_max_size+``の値を増やす必要があるかもしれません。
====
...
* ``+<path_to_xccdf_file.xml>+``はクライアント上のコンテンツファイルの場所を示すパスです。
  例えば、[path]``+/usr/share/xml/scap/ssg/content/ssg-sle15-ds.xml+``です。

* ``+<profile_name>+``+は[command]+``+oscap+``コマンドの追加引数です。
  例えば、``+united_states_government_configuration_baseline+`` (USGCB)を使用します。
...
[guimenu]``+Configuration+``+タブに移動し、[guimenu]+``+Enable Upload of Detailed SCAP Files+``オプションをチェックします。
...
保持期間は[literal]``+Organization Configuration+``セクションで設定できます。
...
. [guimenu]``+SCAP+``の下で、[guimenu]+``+SCAP結果の削除を許可+``をチェックします。

. [guimenu]``+Allow Deletion After+``に日数を入力します（デフォルトは90日です）。
...
すべてのターゲットシステムに[package]``+scap-security-guide+``パッケージをインストールします。
...
. システムグループが作成されたら[guimenu]``+System Groups+``をクリックし、テーブルから[guimenu]+``+Use in SSM+``を選択します。

. From the [guimenu]``+System Set Manager+`` から、menu:Misc[Remote Command]の下で、次のようなBashスクリプトを記述します：
+
