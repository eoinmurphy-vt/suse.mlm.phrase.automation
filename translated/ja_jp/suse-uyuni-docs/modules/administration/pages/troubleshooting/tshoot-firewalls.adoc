...
使用しているファイアウォールが外向きのトラフィックをブロックしている場合、ネットワークリクエストを``+REJECT+``+または+``+DROP+``することがあります。
...
外部トラフィックを許可できない場合は、ファイアウォールを設定して、{productname}からのリクエストに対して+``+DROP+``を使用するのではなく、``+REJECT+``+で処理するようにしてください。
