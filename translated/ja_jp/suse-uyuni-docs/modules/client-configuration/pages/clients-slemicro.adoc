...
ソフトウェアチャンネルは、{webui}またはコマンドラインインタフェースから追加できます。
...
このウィンドウは、クライアントが登録された後、数時間後に開始される場合があります。
登録スクリプトが終了した後、クライアントを手動で再起動することをお勧めします。これにより、登録が迅速に行われ、システムリストにシステムが表示されるようになります。
====
...
トランザクション更新は一般的にいくつかの再起動方法をサポートしています。
{productname}によって管理されているシステムで再起動する場合は、[systemitem]``+systemd+``を使用することをお勧めします。
他の方法を使用すると、望ましくない動作が発生する可能性があります。

{productname}でトランザクションシステムをブートストラップする際、システムがデフォルト構成の場合、[systemitem]``+systemd+``+が再起動方法として設定されます（[literal]+``+REBOOT_METHOD+``）。
このような構成により、{productname}が再起動アクションを制御でき、再起動は即時に行うことも、必要に応じて{productname}でスケジュールすることもできます。


=== 背景情報

デフォルトでは、クライアントインストール中の再起動方法は[option]``+auto+``に設定されています。
[option]``+auto+``+ブート方法では、サービスが実行中の場合、[command]+``+rebootmgrd+``が構成されたポリシーに従ってシステムを再起動するために使用されます。
ポリシーは、即時に再起動するか、メンテナンスウィンドウ中に再起動することができます。
詳細については、[command]``+rebootmgrd+``(8)のマニュアルページを参照してください。
そうでない場合、[command]``+rebootmgrd+``+が実行されていないとき、{productname}は[command]+``+systemctl reboot+``を呼び出します。
