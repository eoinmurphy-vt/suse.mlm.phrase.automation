...
[NOTE]
====
このオプションは、[command]``+virtual-host-gatherer+``ツールで作成されたファイルをインポートするためのものです。
...
. btn:[Create]作成をクリックし、[guimenu]``+ファイルベース+``を選択します。
. [guimenu]``+ファイルベースの仮想ホストマネージャの追加+``セクションで、次のパラメータを使用します:
* [guimenu]``+ラベル+``フィールドにVHMのカスタム名を入力します。
* [guimenu]``+Url+``フィールドに、エクスポートしたJSON設定ファイルへのパスを入力します。
. btn:[Create]をクリックして変更を保存し、VHMを作成します。
. [guimenu]``+仮想ホストマネージャ+``ページで、新しいVHMを選択します。
. [guimenu]``+プロパティ+``ページで、btn:[データの更新]をクリックして、新しいVHMを一覧に反映させます。
...
詳細については、{productname}サーバの[command]``+virtual-host-gatherer+``のマニュアルページを参照してください:
