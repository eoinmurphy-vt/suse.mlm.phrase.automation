[[patch-management]]
= Patch Management
:description: Learn how to create, manage, and deploy custom patches within your organization using the SUSE Multi-Linux Manager Web UI to issue patch alerts, schedule
:revdate: 2022-06-23
...
. In the {productname} {webui}, navigate to menu:Patches[Manage Patches], and click the advisory name of the patch to see the patch details.
...
. To delete a patch, select the patch in the `+Patches Management+` page, and click btn:[Delete Patches].
...
. Navigate to menu:Systems[System Set Manager] and naviagte to the `+Patches+` tab.
