...
:description: Create autoinstallable distributions that can be configured for client installation through various methods
:revdate: 2025-02-06
...
{productname} uses the `+mgradm+` tool to copy the installation file from the source to the server container.
...
Alternatively, you can install the `+tftpboot-installation+` package.
...
This is usually a DVD `+.iso+` image that contains the Linux kernel, an `+initrd+` file, and other files required to boot the operating system in installation mode.



.Procedure: Importing Files from Installation Media
. Use `+mgradm+` to import installation data from the ISO image:
+
----
...
`+mgradm+` is able to automatically detect distribution name and register it to the server.
Provided ISO image needs to contain a `+.treeinfo+` file.
...
. On the {productname} Server, install the package whose name starts with `+tftpboot-installation+`.
  You can determine its exact name with the command `+zypper se tftpboot-installation+`
...
----
. Find the installation files with the command `+ls -d /opt/tftpinstall/usr/share/tftpboot-installation/*+`.
. Copy installation files using `+mgradm+`:
+
----
# mgradm distribution copy /opt/tftpinstall/usr/share/tftpboot-installation/SLE-Micro-5.5-x86_64 SLE-Micro-5.5-x86_64
----
. Take a note of the distribution path reported by `+mgradm+` tool.
  You will need it when you declare the distribution to {productname}.
. After `+mgradm+` tool finishes, you can remove `+/opt/tftpinstall+` directory.
...
If you want to install a different operating system or version on the client, you need to manually get the package `+tftpboot-installation-*+` from the distribution it belongs to.
In the `+Package Search+` input box {productname}, search the packages whose names start with `+tftpboot-installation+`, then look at the package's details.
...
* In the `+Tree Path+` field, enter the path to the installation media saved on your {productname} Server.
* Select the matching `+Base Channel+`.
  This must match the installation media.
* Select the `+Installer Generation+`.
  This must match the installation media.
...
If they are not included, add `+useonlinerepo=1+` to the `+Kernel Options+` field.

The package repositories contain metadata that can be unsigned.
If the metadata is unsigned, add `+insecure=1+` to the `+Kernel Options+` field, or use your own GPG key as explained in xref:client-configuration:autoinst-owngpgkey.adoc[Use Your Own GPG Key].
...
Make sure you use the {sle} installation media, and choose the `+SLE-Product-SUSE-Manager-Proxy-4.3-Pool for x86_64+` base channel.
====
endif::[]
