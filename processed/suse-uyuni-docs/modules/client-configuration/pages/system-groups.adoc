[[system-groups]]
= System Groups
:description: Configure system groups to manage multiple clients by grouping them based on various criteria and performing bulk actions
:revdate: 2024-04-17
...
. Navigate to the `+Target systems+` tab.
...
. Navigate to menu:Systems[System Set Manager], and go to the `+Groups+` tab.
. Locate the group to join and check `+Add+`.
...
You can see which clients are in a group by navigating to menu:Systems[System Groups], clicking the name of the group, and navigating to the `+Systems+` tab.
