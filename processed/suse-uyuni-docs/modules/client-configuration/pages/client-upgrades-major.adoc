...
+
In the `+Distribution Label+` field, enter a name to identify your autoinstallation distribution.
  For example, `+sles15sp6-x86_64+`.
* In the `+Tree Path+` field, enter the path to the installation media you already saved.
...
. When the uploaded profile requires variables to be set, navigate to menu:Systems[Autoinstallation > Profiles], select the profile to edit, and navigate to the `+Variables+` tab.
+
...
It also writes a new `+/boot/grub/menu.lst+` (GRUB Legacy) or `+/boot/grub2/grub.cfg+` (GRUB 2), containing pointers to the new kernel and initrd packages.
