[[ssm]]
= System Set Manager
:description: Learn how to manage multiple clients simultaneously with the System Set Manager (SSM), which creates ephemeral client sets for one-off actions, applies common
:revdate: 2024-11-21
...
When you have selected the clients you want to work with, navigate to menu:Systems[System Set Manager], or click the `+systems selected+` icon in the top menu bar.
...
. Navigate to the `+Channels+` subtab.
. Locate the current base channel in the list, and verify that the number shown in the `+Systems+` column is correct.
    You can click the number in this column to see more details of the clients you are changing.
. Select the new base channel in the `+Desired base Channel+` field, and click btn:[Next].
. For each child channel, select `+No change+`, `+Subscribe+`, or `+Unsubscribe+`, and click btn:[Next].
