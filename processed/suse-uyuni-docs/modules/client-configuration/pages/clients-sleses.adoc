...
:description: Learn how to configure and register SUSE Liberty Linux clients by adding required software channels and syncing them with your SUSE Multi-Linux Manager Server
:revdate: 2026-04-20
...
| {sll} 9.6 EMS | RHEL and Liberty 9.6 EMS Base  | ManagerTools-EL9-Pool for x86_64 and ManagerTools-EL9-Updates for x86_64
...
[IMPORTANT]
====
For {redhat} 10, 9 and 8 clients, add both the Base and AppStream channels.
...
[WARNING]
====
You must use the original installation media from {redhat}.
You must use a major version release (for example `+7+`), not a minor release (for example `+7.9+`).
====
. Copy the base media image to the {productname} Server container host to the `+/tmp/+` directory.
...
+
The destination in the container will be `+/srv/www/distributions/rhel-server-7.0+`.
...
| SSL CA Certificate     | `+redhat-uep+` ({redhat} only)
| SSL Client Certificate | `+Entitlement-Cert-date+` ({redhat} only)
...
Migrating Enterprise Linux (EL) clients to {sll} with the `+liberate+` formula.
