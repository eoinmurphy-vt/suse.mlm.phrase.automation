...
:description: Learn to configure automated installation on SUSE systems with the system.createSystemRecord API call
:revdate: 2024-03-14
...
. On the {productname} Server, at the command prompt, use the `+system.createSystemRecord+` API call.
  In this example, replace `+name+` with the name of your client, `+<profile>+` with the profile label, `+<iface>+` with the name of the interface on the client such as `+eth0+`, and `+<hw_addr>+` with its hardware address such as `+00:25:22:71:e7:c6+`:
+
----
