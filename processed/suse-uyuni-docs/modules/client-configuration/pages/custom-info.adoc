...
:description: Learn how to include customized client information using key-value pairs that can be assigned to specific machines
:revdate: 2024-03-05
...
. In the `+Key Label+` field, add a name for your key.
  Do not use spaces.
  For example, `+intel-x86_64-quadcore+`.
. In the `+Description+` field, provide any additional information required.
...
. In the {productname} {webui}, navigate to `+Systems+`, click the client to apply custom information to, and navigate to the menu:Details[Custom Info] tab.
. Click btn:[Create Value].
. Locate the value you want to apply, and click the key label.
. In the `+Value+` field, provide any additional information.
