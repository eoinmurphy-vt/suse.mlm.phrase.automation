...
:description: Configure import settings to include third-party virtualization providers like VMware in SUSE Multi-Linux Manager
:revdate: 2024-09-03
...
[NOTE]
====
This option is for importing files that have been created with the `+virtual-host-gatherer+` tool.
...
. Export the JSON configuration file by running `+virtual-host-gatherer+` on the VM network.
. Save the produced file to a location accessible by your {productname} Server.
. In the {productname} {webui}, navigate to menu:Systems[Virtual Host Managers].
. Click btn:[Create] and select `+File-based+`.
. In the `+Add a file-based Virtual Host Manager+` section, use these parameters:
...
For more information, see the man page on your {productname} server for `+virtual-host-gatherer+`:
