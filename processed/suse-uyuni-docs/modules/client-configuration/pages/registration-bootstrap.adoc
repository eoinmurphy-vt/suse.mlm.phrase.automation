...
:description: Learn how to create and customize a Bootstrap script for registering clients with the SUSE Multi-Linux Manager Server. (Note I removed the word for as it was
:revdate: 2025-06-10
...
When you create your bootstrap script, add a path to the file containing the public key fingerprint with the `+ORG_GPG_KEY+` parameter.
====
...
== Create a bootstrap script with `+mgr-bootstrap+`

The `+mgr-bootstrap+` command generates custom bootstrap scripts.
A bootstrap script is used by {productname} client systems for simplifying their initial registration and configuration.

The arguments `+--activation-keys+` and `+--script+`, are the only mandatory arguments.
On the {productname} Server, as root at the command line execute it with the mandatory arguments.
Replace `+<ACTIVATION_KEY+` and `+<EDITED_NAME>+` with your values:
...
. The bootstrap script is generated and stored on the server in the `+/srv/www/htdocs/pub/bootstrap+` directory.
...
Ensure that `+Enable SSL+` is checked in the {webui}, or that the setting `+USING_SSL=1+` exists in the bootstrap script.
...
Navigate to menu:Home[Overview] and, in the `+Tasks+` box, click `+Manage Activation Keys+`.
...
. Open `+bootstrap-sles15.sh+` for modification.
...
[NOTE]
====
By default, bootstrap script will try to install `+venv-salt-minion+` if it is available in the bootstrap repository and `+salt-minion+` if there is no {salt} bundle in the bootstrap repository.
It is possible to avoid installing {salt} bundle and keep using `+salt-minion+` if you need it for some reason.
...
. Run this command to execute the bootstrap script on the client; replace `+EXAMPLE.COM+` with the host name of your client:
