...
:description: Configure a bootable CD-ROM or USB key using Cobbler or KIWI tools to deploy SUSE systems, allowing users to create tailored ISO images with specific
:revdate: 2024-11-27
...
The {cobbler} `+buildiso+` command takes parameters to define the name and output location of the boot ISO.
Specifying the distribution with `+--distro+` is mandatory when running `+buildiso+` command.
`+--iso+` is the output location:
...
You can limit which profiles and systems are used with the `+--profiles+` and `+--systems+` options:
...
With `+--esp+` you can enable the built boot ISOs with Secure Boot.
...
. On the container host, execute the following command to get the `+distro+` name:
+
----
mgrctl exec -- cobbler distro list
----
...
. On the container host, execute the following command to get information about the `+distro+` files location:
+
----
...
. See the contents of the boot directory.
  In the example above, the boot partition is the `+/usr/share/tftpboot-installation/SLE-15-SP6-x86_64/boot/x86_64/efi+` file, but this can differ based on the ISO distributor.
