...
:description: Configure clients on the command line using the Salt tools, which involve manually registering them with the Salt minion files and connecting them to the SUSE
:revdate: 2025-03-28
...
* requires that you have installed the `+venv-salt-minion+` (Salt bundle) or the `+salt-minion+` package on the Salt client before registration.
...
. On the Salt client, open the `+minion+` configuration file.
...
. Restart the `+venv-salt-minion+` service:
...
. On the {productname} Server, accept the new client key; replace `+<client>+` with the name of your client:
