...
:description: Learn how to register and configure SL Micro clients for SUSE Multi-Linux Manager server access in order to gain access
:revdate: 2026-06-01
...
https://documentation.suse.com/sles/15-SP7/html/SLES-all/cha-transactional-updates.html.
...
It is recommended to use `+systemd+` for rebooting in systems managed by {productname}.
Using other methods can lead to undesired behavior.

When bootstrapping a transactional system on {productname}, `+systemd+` will be configured as the reboot method (`+REBOOT_METHOD+`), if the system is in its default configuration.
...
With the `+auto+` boot method, `+rebootmgrd+` will be used to reboot the system according to the configured policies if the service is running.
Policies can be to reboot instantly or during a maintenance window.
For more information, see the `+rebootmgrd+`(8) man page.
Otherwise if `+rebootmgrd+` is not running, {productname} will call `+systemctl reboot+`.
