...
:description: Learn how to configure SSH Push using the tunnel contact method in environments where clients cant reach the SUSE Multi-Linux Manager Server directly; instead
:revdate: 2025-02-06
...
SSH Push (with tunnel) (`+ssh-push-tunnel+`) is used in environments where clients cannot reach the {productname} Server directly.
...
To overwrite it, inside the container, you can add a custom port numbers greater than 1024 to `+/etc/rhn/rhn.conf+`:

----
ssh_push_port_https = high_port
----

After changing this configuration parameter you must restart services such as `+salt-secrets-config.service+`, `+tomcat.service+`, and
`+taskomatic.service+`.
