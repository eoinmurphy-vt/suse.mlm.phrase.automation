...
:description: In SUSE Multi-Linux Manager Server environments, learn how to use SSH Push as a contact method when clients cannot reach the server directly
:revdate: 2025-02-06
...
SSH Push (`+ssh-push+`) is used in environments where Salt clients cannot reach the {productname} Server directly.
...
It does not scale well, and consumes more Server resources and network bandwidth than the plain Salt method (`+default+`).
...
All items left of the `+Taskomatic+` block represent processes running on the {productname} client.

image::salt-ssh-contact-taigon.png[scaledwidth=80%]


To use SSH Push, you must have the SSH daemon running on the client, and reachable by the `+salt-api+` daemon running on the {productname} Server.
...
. Check the `+Manage system completely via SSH+` checkbox.
...
. Ensure you have the latest `+spacewalk-taskomatic+` and `+spacewalk-certs-tools+` packages installed on the {productname} Server.
. On each client system, create an appropriate unprivileged user.
. On each client system, edit the `+sudoers+` file:
+
----
sudo visudo
----
...
. Inside the {productname} Server container, edit the `+/etc/rhn/rhn.conf+` configuration file, and add or amend this line to include the unprivileged username:
+
----
ssh_push_sudo_user = <user>
----
+
. After changing this configuration parameter you must restart services such as `+salt-secrets-config.service+`, `+tomcat.service+`, and `+taskomatic.service+`.
