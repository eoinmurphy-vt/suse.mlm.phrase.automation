...
[NOTE]
====
CentOS 7 will fail to onboard due to the fact that the `+/etc/yum.repos.d/CentOS-Base.repo+` has URLs enabled, and thus causing `+yum+` to attempt to find the repositories using fastmirror.
