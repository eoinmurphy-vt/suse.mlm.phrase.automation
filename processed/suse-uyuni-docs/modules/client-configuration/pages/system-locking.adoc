[[system-locking]]
= System Locking
:description: Learn how to prevent actions from occurring on clients using system locks, and discover the benefits of package locking for enhanced software package security
:revdate: 2024-11-21
...
When a client is locked, or put into blackout mode, no actions can be scheduled, execution commands are disabled, and a yellow banner is displayed on the `+System Details+` page.
...
. In the {productname} {webui}, navigate to the `+System Details+` page for the client you want to lock.
. Navigate to the `+Formulas+` tab, check the system lock formula, and click btn:[Save].
. Navigate to the menu:Formulas[System Lock] tab, check `+Lock system+`, and click btn:[Save].
...
.Procedure: Using Package Locks
. On {rhel} 7 systems, install `+yum-plugin-versionlock+`, and on {rhel} 8 or 9 systems, install `+python3-dnf-plugin-versionlock+` as `+root+`.
