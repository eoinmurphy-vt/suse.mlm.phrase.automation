...
:description: Configure and manage virtualization with Nutanix using SUSE Multi-Linux Managers VHM setup and data collection tools, leveraging its capabilities for efficient
:revdate: 2025-02-06
...
Ensure you have installed the `+virtual-host-gatherer-Nutanix+` package on the {productname} Server.
...
. Click btn:[Create] and select `+Nutanix AHV+`.
. In the `+Add a Nutanix AHV Virtual Host Manager+` section, use these parameters:
...
. On the `+Virtual Host Managers+` page select the new VHM.
. On the `+Properties+` page, click btn:[Refresh Data] to inventory the new VHM.
...
If you are using a custom CA authority for your Nutanix SSL certificate, copy the custom CA certificate to `+/etc/pki/trust/anchors+` on the {productname} Server.
...
{productname} ships with a tool called `+virtual-host-gatherer+` that can connect to VHMs using their API, and request information about virtual hosts.
...
Log files for the `+virtual-host-gatherer+` tool are located at `+/var/log/rhn/gatherer.log+`.
