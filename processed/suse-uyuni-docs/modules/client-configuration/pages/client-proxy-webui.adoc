...
:description: Configure clients in the SUSE Multi-Linux Manager Web UI by registering them with a proxy server via the Systems Bootstrapping section
:revdate: 2024-11-21
...
. In the `+Authentication Method+` field, select the authentication method to use for bootstrapping the client.
+
...
. In the `+Proxy+` field, select the proxy server you want to register to.
