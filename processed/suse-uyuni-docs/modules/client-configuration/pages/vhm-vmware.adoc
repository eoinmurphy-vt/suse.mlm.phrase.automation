...
:description: Configure VMware vSphere virtual machines and inventory hosts with SUSE Multi-Linux Manager using a virtual host manager (VHM) set up on the server
:revdate: 2024-09-03
...
. Click btn:[Create] and select `+VMware-based+`.
. In the `+Add a VMware-based Virtual Host Manager+` section, use these parameters:
...
To correct the problem, extract the certificate from the ESXi server, and copy it to `+/etc/pki/trust/anchors+`.
...
. Copy the certificate files to your {productname} Server, and save them to the `+/etc/pki/trust/anchors/+` directory.
