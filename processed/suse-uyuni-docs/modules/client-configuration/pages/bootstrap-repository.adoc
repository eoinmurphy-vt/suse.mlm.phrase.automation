...
:description: Learn how to configure and manage bootstrap repositories for client registration during bootstrapping, which includes settings for automated creation
:revdate: 2025-02-06
...
To enable this behavior, add or edit this value in `+/etc/rhn/rhn.conf+`:
+
----
...
If you are using custom channels, you can use the `+--with-custom-channels+` option with the `+mgr-create-bootstrap-repo+` command.
