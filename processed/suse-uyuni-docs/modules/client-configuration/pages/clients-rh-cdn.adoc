...
:description: Learn how to register Red Hat Enterprise Linux clients with the Red Hat Content Delivery Network at redhat.com using subscription-manager, then manage your
:revdate: 2026-03-17
...
[NOTE]
====
Your entitlement certificate and key both have a file extension of `+.pem+`.
The key also has `+key+` in the filename.
====
+
...
* In the `+Type+` field, select `+SSL+`.
* In the `+Select file to upload+` field, browse to the location where you saved the {redhat} certificate, and select the certificate file.
. Click btn:[Create Key].
...
----
. Click btn:[Create Configuration File]
. Take note of the value of the field `+Salt Filesystem Path+`.
. Click on the name of the Configuration Channel.
. Click on `+View/Edit 'init.sls' File+`
* In the `+File Contents+` field, type:
+
----
...
. Click btn:[Create Group].
. Click `+States+` tab.
. Click `+Configuration Channels+` tab.
. Type `+subscription-manager: disable yum plugins+` at the search box.
. Click btn:[Search] to see the state.
. Click the checkbox for the state at the `+Assign+` column.
...
. Navigate to the `+Groups+` tab, and the `+Join+` subtab.
. Check `+Select rhel-systems+`.
...
[NOTE]
====
For the {redhat} custom channels, if you want to check the `+Enable GPG Check+` field, you need to populate the `+GPG Key URL+` field.
You can use the file URL of the GPG key stored in the directory `+/etc/pki/rpm-gpg+` of the {redhat} minion.
