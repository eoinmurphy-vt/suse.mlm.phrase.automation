...
:description: Register clients to a proxy using the command line by following this procedure and executing the specified commands
:revdate: 2024-11-21
...
For SLE 12 based clients, you also must have activated the `+Advanced Systems Management+` module.
...
----
. On the server, accept the new client key; replace `+<client>+` with the name of your client:
+
----
salt-key -a '<client>'
----
