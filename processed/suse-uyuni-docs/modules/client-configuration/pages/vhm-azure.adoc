...
:description: Configure a virtual host manager for Azure and gather instances from Microsoft Azure using the SUSE Multi-Linux Manager
:revdate: 2025-02-06
...
. Click btn:[Create] and select `+Azure+` from the drop-down menu.
. In the `+Add an Azure Virtual Host Manager+` section, use these parameters:
...
* In the `+Zone+` field, type the zone your VM is located in. For example, for West Europe, enter `+westeurope+`.
