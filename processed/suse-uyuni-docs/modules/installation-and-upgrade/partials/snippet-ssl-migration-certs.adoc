...
To use certificates from an external PKI (certificate authority), you can provide all certificate files when running `+{migration-command}+` using the `+--ssl*+` flags.
...
Any `+--ssl*+` flag not specified uses the corresponding value from the source server.
...
The `+--ssl-db-*+` flags must be provided even when using the same CA,
...
In that case, the same certificate files are used for both `+--ssl-*+` and `+--ssl-db-*+` flags.
The example below uses separate file names for clarity.

.Example migration command with all `+--ssl*+` flags
