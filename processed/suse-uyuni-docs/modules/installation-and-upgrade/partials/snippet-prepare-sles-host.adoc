...
[IMPORTANT]
====
Make sure that package `+podman+` is installed.
Additionally,  on the server `+mgradm+` and `+mgradm-bash-completion+` or on the proxies, `+mgrpxy+` and `+mgrpxy-bash-completion+` also need to be installed.
====
