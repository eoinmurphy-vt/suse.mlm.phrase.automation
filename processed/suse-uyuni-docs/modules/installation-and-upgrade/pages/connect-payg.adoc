...
:description: Connect your cloud instance to SUSE Multi-Linux Manager for access to additional products and repositories
:revdate: 2026-05-06
...
. If the authentication data for the instance are correct, the column `+Status+` shows "Credentials successfully updated."
...
For example, if the instance has the {sles}{nbsp}15 SP1 product installed, {sles}{nbsp}15 SP2, {sles}{nbsp}15 SP3, {sles}{nbsp}15 SP4 and {sles}{nbsp}15 SP5 are automatically shown in `+Admin > Setup Wizard > Products+`.
...
After the success message you can verify the newly added channels in the {webui}, by navigating to `+Software > Channel List > All+`. 

To monitor the syncing progress of each channel, check the log files in the `+/var/log/rhn/reposync+` directory on the {productname} Server.
...
The status of the {payg} instance credentials collect is shown in the column `+Status+` or on the instance details page.
...
* removing cloud-specific entries from `+/etc/hosts+`
====
