...
However, if custom persistent storage is required for your infrastructure, use the `+mgr-storage-proxy+` tool.
...
. In the `+SSL certificate+` selection list choose if new server certificate should be generated for {productname} Proxy or an existing one should be used.
