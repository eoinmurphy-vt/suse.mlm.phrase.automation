...
To migrate a SUSE Manager 4.3 Proxy to {productname} {productnumber}, you require a new machine with {sl-micro} {microversion} or {sles} {bci-mlm} and `+mgrpxy+` installed.
...
- Or use API call `+system/provisionSystem+` to schedule the migration. For example execute following snippet from the {productname} host:
+
[code]
----
mgrctl api login
...
mgrpxy start
----
. Once the proxy is operational, the {productname} Server will automatically deploy the backed up configuration during the next `+Hardware Refresh+` step.
...
In the {productname} {productnumber} server container, run `+configure-tftpsync.sh+` with the list of remaining 4.3 proxies as arguments.
If no 4.3 proxies remain, run `+configure-tftpsync.sh+` with no arguments.

In the {productname} {productnumber} server container, manually remove the relevant proxy from the `+proxies+` setting in the `+/etc/cobbler/settings.yaml+` file.
