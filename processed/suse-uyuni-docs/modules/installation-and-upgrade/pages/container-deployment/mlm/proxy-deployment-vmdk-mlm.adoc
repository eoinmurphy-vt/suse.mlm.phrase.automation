...
. Download {productname} Proxy `+.vmdk+` file then transfer a copy to your VMware storage.

. Make a copy of uploaded `+.vmdk+` file using VMware web interface.
  This will convert provided `+.vmdk+` file to the format suitable for vSphere hypervisor.

. Create and name a new virtual machine based on the Guest OS Family `+Linux+` and Guest OS Version {sle} 15 (64-bit).

. Add an additional `+Hard Disk 2+` of 100 GB (or more).
