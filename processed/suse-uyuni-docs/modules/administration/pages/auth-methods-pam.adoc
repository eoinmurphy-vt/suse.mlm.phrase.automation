...
:description: Configure SSSD in SUSE Multi-Linux Manager to integrate network-based authentication systems using PAM for secure user management
:revdate: 2025-01-08
...
====
. Check the `+Pluggable Authentication Modules (PAM)+` checkbox.
...
+
.. Edit `+/etc/sssd/sssd.conf+` according to your configuration.
...
Your domain name
`+$ad_server+`::
FQDN of the AD server if it is not auto-detected from the `+$domain+`
`+$uyuni-hostname+`:
The name of the machine this AD client is supposed to be known.
If not set, it will be `+uyuni-server.mgr.internal+`.
