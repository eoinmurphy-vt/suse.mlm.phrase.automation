...
:description: Configure monitoring for your SUSE Multi-Linux Manager environment with Prometheus and Grafana to record real-time metrics in a time-series database
:revdate: 2026-05-19
...
[IMPORTANT]
====
* To access a shell inside the {productname} Server container run `+mgrctl term+` on the container host, or to execute one command run `+mgrctl exec <options> -- <command>+`.
...
[role=procedure]
____
. In the {productname} {webui}, open the details page of the system where Prometheus is to be installed, and navigate to the `+Formulas+` tab.

. Check the `+Prometheus+` checkbox to enable  monitoring formulas, and click btn:[Save].
...
. Check that the Prometheus interface loads correctly. In your browser, navigate to the URL of the server where Prometheus is installed, on port 9090 (for example, `+http://example.com:9090+`).
...
. Check that the Prometheus interface loads correctly.
  In your browser, navigate to the URL of the server where Prometheus is installed, on port 9090 (for example, `+http://example.com:9090+`).

. Open the configuration file at `+/etc/prometheus/prometheus.yml+` and add this configuration information.
...
. In the {productname} {webui}, open the details page of the system where Grafana is to be installed, and navigate to the `+Formulas+` tab.

. Check the `+Grafana+` checkbox to enable  monitoring formulas, and click btn:[Save].
...
. Check that the Grafana interface is loading correctly. In your browser, navigate to the URL of the server where Grafana is installed, on port 3000 (for example, `+http://example.com:3000+`).
...
. In your browser, navigate to the URL of the server where Grafana is installed, on port 3000 (for example, `+http://example.com:3000+`).
...
. Navigate to the URL of your Prometheus server, on port 9090 (for example, `+http://example.com:9090+`)
...
. Check the `+Enabled+` checkbox on the `+Prometheus Exporters+` formula.
...
. Select the exporters you want to enable and customize arguments according to your needs.
    The `+Address+` field accepts either a port number preceded by a colon (`+:9100+`), or a fully resolvable address (`+example:9100+`).
...
. In the {productname} {webui}, open the details page of the system to be monitored, and navigate to the `+Formulas+` tab.

. Check the `+Prometheus Exporters+` checkbox to enable the exporters formula, and click btn:[Save].

. Navigate to the `+Prometheus Exporters+` tab in the top menu.

. Check the `+Enable reverse proxy+` option, and enter a valid reverse proxy port number.
  For example, `+9999+`.
...
Ensure that the `+set-cname+` parameter is the fully qualified domain name (FQDN) of your {salt} client.
You can use the `+set-cname+` parameter multiple times if you require multiple aliases:
...
.. Copy `+server.crt+` and `+server.key+` files from the server container to the host:
...
.. Copy `+server.crt+` and `+server.key+` files from the host to the monitoring client:
