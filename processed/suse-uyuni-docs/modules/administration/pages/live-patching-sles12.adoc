...
:description: Enable Live Patching on SLES12 systems by following the setup, patch application, and verification procedures
:revdate: 2024-02-01
...
. Select the client you want to manage with Live Patching from menu:Systems[Overview], and on the system details page navigate to the menu:Software[Packages > Install] tab.
    Search for the `+kgraft+` package, and install it.
+
...
. To check that live patching has been enabled correctly, select the client from menu:Systems[System List], and ensure that `+Live Patching+` appears in the `+Kernel+` field.
