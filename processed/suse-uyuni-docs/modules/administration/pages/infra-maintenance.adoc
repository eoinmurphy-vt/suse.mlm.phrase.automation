...
For information on using [command]``+transactional-update+``, see https://documentation.suse.com/sle-micro/6.1/html/Micro-transactional-updates/transactional-updates.html.
...
Updating [package]``+venv-salt-minion+``+, [package]+``+zypper+``, and other related management package on clients is not a strict requirement, but it is a best practice in general.
...
To avoid this {suse} makes sure that [package]``+venv-salt-minion+`` will always be updated safely.
