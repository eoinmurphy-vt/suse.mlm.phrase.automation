...
In the {webui}, on the menu:System Details[Properties] page of the registered system, you must enable the `+Ansible Control Node+` system type of the `+Add-on System Types+` list.

// FIXME: check it!
Enabling the `+Ansible Control Node+` system type ensures that the `+ansible+` package is installed on the system by adding it in the highstate and activates the Ansible features under the menu:System Details[Ansible] tab.
...
You can also pass variables to the playbook execution to override previously defined variables. 
To pass variables, locate the [guimenu]``+Playbook Content+`` section and click btn:[Edit variables].
In the [guimenu]``+Edit Variables+`` dialog, you can add or edit variables as a list, dictionary, string, or boolean.
Alternatively, click btn:[Edit YAML] to edit the variables directly as YAML. Click btn:[Save] to apply the variables to the playbook content.
