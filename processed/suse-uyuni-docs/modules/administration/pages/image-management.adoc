...
You can manage [path]`+/etc/gitconfig+` on build hosts through Configuration Management, the same way as on any other managed system.
...
To ensure the correct version is used for SLES 15 profiles, you must define the [literal]``+kiwi_image+`` custom info key with the following value:

* [literal]``+Key+``: kiwi_image
* [literal]``+Value+``+: [path]+``+registry.suse.com/bci/kiwi:9+``

If this key is not set, the system defaults to the latest version (e.g., [path]``+registry.suse.com/bci/kiwi:10.2+``), which may result in build issues for SLES 15 profiles.
