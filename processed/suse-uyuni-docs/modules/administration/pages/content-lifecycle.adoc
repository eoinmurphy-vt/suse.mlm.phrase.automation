...
:description: Configure content lifecycle management to customize and test packages prior to updating production clients
:revdate: 2025-02-05
...
You can disable the automatic selection for cloned channels by adding the following option in `+/etc/rhn/rhn.conf+`:
...
. In the `+Label+` field, enter a label for your project.
    The `+Label+` field only accepts lowercase letters, numbers, periods, hyphens, and underscores.
...
. In the `+Sources+` dialog, select the source type, and select a base channel for your project.
...
. In the `+Filters+` dialog, select the filters you want to attach to the project.
    To create a new filter, click btn:[Create new Filter].

. Click btn:[Add Environment].

. In the `+Environment Lifecycle+` dialog, give the first environment a name, a label, and a description, and click btn:[Save].
    The `+Label+` field only accepts lowercase letters, numbers, periods, hyphens, and underscores.
...
. In the `+Prefix+` field, type a name prefix. This value will be prepended to the name of every individual filter created by the template. If the template is being applied in the context of a project, this field will be prefilled with the project label.

. In the `+Template+` field, select `+Live patching based on a SUSE product+`.

. In the `+Product+` field, select the product you wish to set up live patching for.

. In the `+Kernel+` field, select a kernel version from the list of versions available in the selected product. The filter to deny the later regular kernel patches will be based on this version.
...
. In the `+Prefix+` field, type a name prefix.
...
. In the `+Template+` field, select `+Live patching based on a specific system+`.

. In the `+System+` field, select a system from the list, or start typing a system name to narrow down the options.

. In the `+Kernel+` field, select a kernel version from the list of versions installed in the selected system.
...
. In the `+Filters+` section, click btn:[Attach/Detach Filters], and then click btn:[Create New Filter].
...
. In the `+Template+` field, select `+AppStream modules with defaults+`.

. In the `+Channel+` field, select a modular channel to get the modules from. In this dropdown, only the modular channels are displayed.

. Click btn:[Save] to create the filters.

. Scroll to the `+Filters+` section to see the newly attached AppStream filters.
...
To manually synchronize the patch later for all channels sharing the patch, navigate to menu:Software[Manage > Channels], click the channel you want to synchronize and navigate to the `+Sync+` subtab.
...
. You can monitor build progress in the `+Environment Lifecycle+` section.
...
. In the `+Environment Lifecycle+` section, locate the environment to promote to its successor, and click btn:[Promote].
...
To add clients to the environment, assign the base and child software channels to your client using menu:Software[Software Channels] in the `+System Details+` page for the client.
