[[support]]
= Support
:description: Navigate to the Support sub-tab, entering your service request number and upload region, and then schedule data collection for the targeted system
...
. Then navigate to the `+Details+` tab and select the `+Support+` sub-tab.

. In the field `+Support Case Number+` enter the service request number created above.

. In the field `+Upload Region+` select option `+EU+` or `+US+` depending on the server where you want to upload your data.

. In the field `+Command-line Arguments+` you can enter options for tool which is used to collect the data from the target system.
  Which tool will be used can be found in the tip below this field.

. In the field `+Earliest+` select the time of running this action.
...
[NOTE]
====
To upload the support data from the main {productname} Server please still use `+mgradm support config+` from the container host.
====
