...
:description: Configure your Salt clients to validate with a specific master by entering that masters fingerprint in the minion configuration file
:revdate: 2024-12-12
...
On your client, open the `+/etc/salt/minion.d/custom.conf+` or `+/etc/venv-salt-minion/minion.d/custom.conf+` configuration file.
