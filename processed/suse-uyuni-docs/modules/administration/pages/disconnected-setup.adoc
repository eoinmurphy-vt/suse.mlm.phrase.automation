[[disconnected-setup]]
= Disconnected Setup
:description: Set up and manage your SUSE Multi-Linux Manager in a disconnected environment using either RMT or SMT
:revdate: 2025-05-27
...
. Export the synchronized data to your removable storage.
    In this example, the storage medium is mounted at `+/mnt/usb+`:
+
----
...
You can change RMT user settings in the `+cli+` section of `+/etc/rmt.conf+`.
====
...
====
Ensure that the external storage is mounted to a directory that is writeable by the RMT user.
You can change SMT user settings in `+/etc/smt.conf+`.
====
...
[NOTE]
====
Be aware that if `+server.susemanager.fromdir+` is set, {productname} will not be able to check if {scclongform} credentials are valid or not.
