...
* On the `+Manage realms+` page, click the btn:[Create realm] button in the top-right corner.
...
* In the `+Client ID+` field, enter the {productname} Metadata endpoint.
  For example: `+https://<FQDN_MLM>/rhn/manager/sso/metadata+`
* Click btn:[Next].
* In the `+Valid redirect URIs+` field, enter the {productname} Assertion Consumer Service (ACS) endpoint.
...
** In both the `+Assertion Consumer Service POST Binding URL+` and `+Assertion Consumer Service Redirect Binding URL+` fields, enter the {productname} ACS endpoint.
  For example: `+https://<FQDN_MLM>/rhn/manager/sso/acs+`
** In the `+Logout Service POST Binding URL+` field, enter the {productname} SLS endpoint.
  For example: `+https://<FQDN_MLM>/rhn/manager/sso/sls+`
** In the `+Logout Service Redirect Binding URL+` field, enter the root URL of your {productname} instance.
...
* Toggle `+Single Role Attribute+` to `+On+`, and click btn:[Save].
* Go back to menu:Clients[], click your {productname} client ID (`+https://<FQDN_MLM>/rhn/manager/sso/metadata+`), and click the `+Client scopes+` tab.
...
* Click btn:[Add user].
* In the `+Username+` field, enter a username that matches an existing {productname} user (for example, the administrative user).
* Click btn:[Create].
* Click the `+Credentials+` tab, click btn:[Set password], set a password for the user, and toggle `+Temporary+` to `+Off+`, click btn:[Save] and confirm by clicking btn:[Save password].
____
...
[role=procedure]
____
. On the {productname} Server, open the `+/etc/rhn/rhn.conf+` configuration file.
