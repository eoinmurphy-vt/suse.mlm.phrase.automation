...
:description: Set up and manage live patching channels for clients using either Spacewalks content lifecycle management functionality or the
:revdate: 2025-06-30
...
----
[subs=attributes]
. Check that `+dev-sles15-{sp-vert}-updates-x86_64+` is available in your channel list.
...
. Check the current kernel version by selecting the client from menu:Systems[System List], and taking note of the version displayed in the `+Kernel+` field.
. In the {productname} {webui}, select the client from menu:Systems[Overview], navigate to the menu:Software[Manage > Channels] tab, and select `+dev-sles15-{sp-vert}-updates-x86_64+`.
      Navigate to the `+Patches+` tab, and click btn:[List/Remove Patches].
...
. Check the new `+test-sles15-{sp-vert}-pool-x86_64+` custom channel to change the base channel, and check both corresponding live patching child channels.
