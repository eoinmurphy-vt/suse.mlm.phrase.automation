...
:description: Configure single sign-on (SSO) for secure access to multiple applications using a single set of credentials
:revdate: 2024-05-06
...
The {productname} user must be created before you activate single sign-on.
====
...
----
+
To find all the occurrences you need to change, search in the file for the placeholders `+YOUR-PRODUCT+` and `+YOUR-IDP-ENTITY+`.
