...
= Repository Metadata
:description: Learn about enabling repository metadata signing and deploying GPG keys on your server and for all clients
:revdate: 2024-12-16
...
[role=procedure]
____
. As the root user, use the `+gpg+` command to generate a new key:
...
. At the prompts, select `+RSA+` as the key type, with a size of 2048 bits, and select an appropriate expiry date for your key.
    Check the details for your new key, and type `+y+` to confirm.
...
. At the prompts, enter a name and email address to be associated with your key.
    ...
    When you are happy with the user identity, type `+O+` to confirm.
...
. Add the password for your keyring to the `+/etc/rhn/signing.conf+` configuration file, by opening the file in your text editor and adding this line:
...
. Enable metadata signing with the `+mgr-sign-metadata-ctl+` command:
...
You can also use the `+mgr-sign-metadata-ctl+` command to perform other tasks.
Use `+mgr-sign-metadata-ctl --help+` to see the complete list.
