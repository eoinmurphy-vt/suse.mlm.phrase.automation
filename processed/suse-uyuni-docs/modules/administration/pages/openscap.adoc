...
* ``+<path_to_xccdf_file.xml>+`` is the path to the content file location on the client.
  For example, [path]``+/usr/share/xml/scap/ssg/content/ssg-sle15-ds.xml+``.
...
You can configure the retention period in the [literal]``+Organization Configuration+`` section.
...
. Under [guimenu]``+SCAP+``+, check [guimenu]+``+Allow Deletion of SCAP Results+``.

. Enter a number of days in [guimenu]``+Allow Deletion After+`` (default is 90).
