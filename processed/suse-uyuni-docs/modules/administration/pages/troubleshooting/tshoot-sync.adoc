...
:description: Configure synchronization settings to troubleshoot connection timeouts, GPG key mismatches, and checksum errors
:revdate: 2025-07-08
...
You can find out if this is the case by opening `+/var/log/rhn/reposync+` and looking for an error like this:
+
----
...
----
+
You can resolve this error by specifying `+reposync_timeout+` and `+reposync_minrate+` configuration values in `+/etc/rhn/rhn.conf+`. 
By default, when less than 1000 bytes per second are transferred in 300 secs, the download is aborted.
You can adjust the number of bytes per second with `+reposync_minrate+`, and the number of seconds to wait with `+reposync_timeout+`.


Manually Trusting the Key During reposync::
It is possible that in some cases when `+reposync+` is run, you may need to accept the GPG key manually.
For example:
+
----
