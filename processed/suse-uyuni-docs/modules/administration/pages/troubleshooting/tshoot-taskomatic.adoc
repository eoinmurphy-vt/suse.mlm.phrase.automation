...
= Troubleshooting Taskomatic
:description: Learn about troubleshooting and resolving issues with Taskomatic, a centralized repository management system
:revdate: 2024-05-15
...
In this case, the `+zypper ref+` command shows an error like this:
...
. On the {productname} Server, check the `+/var/log/rhn/rhn_taskomatic_daemon.log+` file to determine if any metadata regeneration processes are still running, or if a crash occurred.
. Restart taskomatic:
+
----
