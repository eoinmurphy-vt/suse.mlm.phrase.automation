...
:description: Resolve duplicate machine IDs and properly register cloned clients in SUSE Multi-Linux Manager, then learn the steps for doing so
:revdate: 2025-04-24
...
.. On the cloned machine, change the hostname and IP addresses.
    Make sure `+/etc/hosts+` contains the changes you made and the correct host entries.
...
... If the cloned machine also has a folder in `+/var/log/journal/+` it needs to be renamed accordingly to the new machine ID. If names do not match, `+journalctl+` could not retrieve any log and `+podman logs+` would not show anything.
...
* {rhel} uses the machine ID to generate kernel entries in `+/boot/loader/entries+`. 
...
.. Re-register the clients.
   Each client now has a different `+/etc/machine-id+` and should be correctly displayed on the `+System Overview+` page.
