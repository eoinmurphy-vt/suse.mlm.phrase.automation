...
:description: Configure RPC connection timeouts to prevent package downloads and batch jobs from hanging, by adjusting the maximum time allowed in the relevant configuration
:revdate: 2022-10-18
...
. On the {productname} Server, open the `+/etc/rhn/rhn.conf+` file and set a maximum timeout value (in seconds):
...
. On the {productname} Proxy, open the `+/etc/uyuni/proxy/config.yaml+` file and set a maximum timeout value (in seconds).
...
. On a {sles} client that uses zypper, open the `+/etc/zypp/zypp.conf+` file and set a maximum timeout value (in seconds):
...
. On a {rhel} client that uses yum, open the `+/etc/yum.conf+` file and set a maximum timeout value (in seconds):
