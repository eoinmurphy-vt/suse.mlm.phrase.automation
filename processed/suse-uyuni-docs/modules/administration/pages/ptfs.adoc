...
:description: Configure and deploy Program Temporary Fixes (PTFs) using detailed procedures and step-by-step instructions for the SUSE Multi-Linux Manager
:revdate: 2024-11-29
...
. Enter `+mgr-sync list channel+` and look for channels starting with your {scc} account name and `+ptfs+` in its name.
...
. Navigate to the menu:Systems[Software > Packages > Software Channels] and select the `+PTF channel+`.
...
. Click btn:[Install PTF], and `+Confirm Program Temporary Fixes (PTFs) Installation+` with btn:[Confirm].
...
. Click btn:[Remove PTFs], and on the `+Confirm Program Temporary Fixes (PTFs) Removal+` page click btn:[Confirm].
. To see the results, navigate to menu:Systems[Events > History].


[NOTE]
====
Removing PTFs requires a special version of `+libzypp+` and `+zypper+` to be installed on the client system.
Check `+zypper --help+` to confirm whether `+removeptf+` is supported.
