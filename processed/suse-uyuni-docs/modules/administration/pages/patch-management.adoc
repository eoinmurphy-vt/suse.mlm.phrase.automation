[[patch-management]]
= Patch Management
:description: Learn how to manage patches effectively using SUSE Multi-Linux Managers features for syncing and propagating retracted patches across channels
:revdate: 2022-02-17
...
A retracted patch or package cannot be installed on systems with {productname}. The only way to install a retracted package, is to do it manually with `+zypper install+` and specifying the exact package version.
For example:
----
...
.Example:
. Consider two Content Lifecycle Management projects `+prj1+` and `+prj2+`
. Both of these projects have 2 environments `+dev+` and `+test+`
