...
:description: Configure live patching on SLES15 systems by following setup, patch application, and verification procedures
:revdate: 2024-02-01
...
On SLES{nbsp}15 systems and newer, live patching is managed by the `+klp livepatch+` tool.
...
. Select the client you want to manage with Live Patching from menu:Systems[Overview], and navigate to the menu:Software[Packages > Install] tab.
    Search for the `+kernel-livepatch+` package, and install it.
+
...
. To check that live patching has been enabled correctly, select the client from menu:Systems[System List], and ensure that `+Live Patch+` appears in the `+Kernel+` field.
...
. Select any patch with a synopsis reading `+Important: Security update for the Linux kernel+`.
