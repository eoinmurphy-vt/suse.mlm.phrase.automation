...
:description: Configure self-signed SSL certificates in the SUSE Multi-Linux Manager to enable secure communication among its components
:revdate: 2025-05-08
...
Ensure that the `+set-cname+` parameter is the fully qualified domain name of your {productname} Server.
You can use the `+set-cname+` parameter multiple times if you require multiple aliases.

The private key and the server certificate can be found inside the server container in the directory `+/root/ssl-build/susemanager/+` as `+server.key+` and `+server.crt+`.
