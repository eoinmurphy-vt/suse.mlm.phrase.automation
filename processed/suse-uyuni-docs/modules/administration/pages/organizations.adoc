[[organizations]]
= Organizations
:description: Learn how to configure organizations in SUSE Multi-Linux Manager for managing user access, permissions, and authentication methods
:revdate: 2022-01-13
...
. In the `+Create Organization+` dialog, complete these fields:
* In the `+Organization Name+` field, type a name for your new organization.
    The name should be between 3 and 128 characters long.
* In the `+Desired Login+` field, type the login name you want to use for the organization's administrator.
...
* In the `+Desired Password+` field, type a password for the new organization's administrator.
...
Navigate to the `+Users+` tab to view the list of all users associated with the organization, and their role.
Clicking a username takes you to the `+Users+` menu to add, change, or delete users.
...
Navigate to the `+Configuration+` tab to manage the configuration of your organization.
...
Navigate to the `+States+` tab to manage Salt states for all clients in your organization.
