[[tuning-changelogs]]
= Tuning Changelogs
:description: Configure changelog metadata in SUSE Multi-Linux Manager, limiting downloadable data and saving disk space by setting a parameter value between 1 and unlimited
:revdate: 2024-05-10
...
This configuration option is in the `+/etc/rhn/rhn.conf+` configuration file.
The parameter defaults to `+20+`.
Changing this value to `+0+` will provide an unlimited number of entries.
