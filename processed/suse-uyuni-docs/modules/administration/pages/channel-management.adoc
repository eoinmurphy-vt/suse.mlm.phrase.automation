[[channel-management]]
= Channel Management
:description: Learn how to configure and manage software channels in SUSE Multi-Linux Manager by grouping packages into base and child channels, assigning clients to base
:revdate: 2025-02-28
...
. Locate the `+Per-User Subscription Restrictions+` section and check `+Only selected users within your organization may subscribe to this channel+`.
  Click btn:[Update] to save the changes.
. Navigate to the `+Subscribers+` tab and select or deselect users as required.
