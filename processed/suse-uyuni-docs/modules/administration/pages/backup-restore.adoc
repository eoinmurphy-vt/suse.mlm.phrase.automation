[[backup-restore]]
= Backup and Restore
:description: Learn to configure SUSE Multi-Linux Manager backups for data consistency to enable quick recovery in the event of a system failure
:revdate: 2025-06-23
...
With the built-in backup and restore solution (`+mgradm backup+`) you create {productname} backups.
...
[NOTE]
====
With the advent of the built-in solution, the old method with the `+smdba+` backup tool is deprecated.
If you migrated from an old system with `+smdba+` to the new solution, you must disable the old funtionality and remove the old backup archives.
====
...
. On the container host, as root, edit `+/var/lib/containers/storage/volumes/var-pgsql/_data/postgresql.conf+` and set these options:
+
----
archive_mode = off
archive_command = '/bin/true'
...
The most comprehensive method for backing up your {productname} installation is to use `+mgradm backup create+` command.
...
`+mgradm backup+` command uses internal list of {productname} volumes.
...
[role=procedure]
____
. Re-synchronize your {productname} repositories using either the {productname} {webui}, or with the `+mgr-sync+` tool at the command prompt in the container.
  You can choose to re-register your product, or skip the registration and SSL certificate generation sections.

. On the container host, check whether you need to restore `+/var/lib/containers/storage/volumes/var-spacewalk/_data/packages/+`.
  If `+/var/lib/containers/storage/volumes/var-spacewalk/_data/packages/+` was not in your backup, you need to restore it.
  If the source repository is available, you can restore `+/var/lib/containers/storage/volumes/var-spacewalk/_data/packages/+` with a complete channel synchronization:
