...
:description: Learn how to use content lifecycle management examples to build a personalized approach to patch cycle implementation
:revdate: 2025-01-30
...
. In the `+Filter Name+` field, type a name for your filter.
    For example, `+Exclude patches by date+`.

. In the `+Filter Type+` field, select `+Patch (Issue date)+`.

. In the `+Matcher+` field, `+later or equal+` is autoselected.
...
. Select the new `+Exclude patches by date+` filter.
...
=== Apply a filter to a new project build
...
. In the `+Filter Name+` field, enter a name for the filter.
    For example, `+Exclude openjdk patch+`.

. In the `+Filter Type+` field, select `+Patch (Advisory Name)+`.

. In the `+Matcher+` field, select `+equals+`.

. In the `+Advisory Name+` field, type a name for the advisory.
    For example, `+SUSE-15-2019-1807+`.

. Click btn:[Save].

. Navigate to menu:Content Lifecycle[Projects] and select your project.

. Click btn:[Attach/Detach Filters] link, select `+Exclude openjdk patch+`, and click btn:[Save].

____

When you rebuild the project with the btn:[Build] button, the new filter is used together with the `+by date+` filter we added before.
...
. In the `+Filter Name+` field, type a name for the filter.
    For example, `+Include kernel security fix+`.

. In the `+Filter Type+` field, select `+Patch (Advisory Name)+`.

. In the `+Matcher+` field, select `+equals+`.

. In the `+Advisory Name+` field, type `+SUSE-15-2019-2071+`, and check `+Allow+`.

. Click btn:[Save] to store the filter.

. Navigate to menu:Content Lifecycle[Projects] and select your project from the list.

. Click btn:[Attach/Detach Filters], and select `+Include kernel security patch+`.
...
[role=procedure]
____
. In the `+by date+` field, change the date of the filter to the next month.
...
. In the `+Filter Name+` field, type a name for your filter.
    For example, `+Exclude kernel greater than 4.12.14-150.17.1+`.

. In the `+Filter Type+` field, select `+Patch (Contains Package)+`.

. In the `+Matcher+` field, select `+version greater than+`.

. In the `+Package Name+` field, type `+kernel-default+`.

. Leave the `+Epoch+` field empty.

. In the `+Version+` field, type `+4.12.14+`.

. In the `+Release+` field, type `+150.17.1+`.
...
. Select `+Exclude kernel greater than 4.12.14-150.17.1+`, and click btn:[Save].
...
* In the `+Module Name+` field, type a module name.
    For example, `+postgresql+`.

* In the `+Stream+` field, type the name of the desired stream.
    For example, `+10+`.
