...
:description: Learn how to manage online database backups for the {productname} server using `+mgradm+`.
:revdate: 2026-03-17
...
This guide describes how to manage online database backups for the {productname} server using `+mgradm+`.
The backup system is based on {postgresql} continuous archiving Write-Ahead Logging (WAL).
...
Only full recovery is supported by `+mgradm+` tool. 
