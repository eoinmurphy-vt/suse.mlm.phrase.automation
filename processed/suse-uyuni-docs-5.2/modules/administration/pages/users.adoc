...
[NOTE]
====
The `+Users+` menu is only available if you are logged in with the {productname} administrator account.
====
...
This field is enable only of the `+Require Special Characters+` is selected.
Use it to specify which special characters are allowed, for example `+!@#$%&*+`, and others.
...
To change a user's {productname} Administrator role, navigate to menu:Admin[Users] and check or uncheck `+{productname} Admin?+` as required.
...
With Role-Based Access Control in {productname}, you can create custom roles (referred to as Access Groups in the {webui}) to fine-tune user permissions.

To manage and create custom roles, navigate to menu:Admin[Access Control] in the {webui} and click btn:[Create Access Group].

Refer to xref:administration:role-based-access-control.adoc[] for more detailed information about these roles.
...
To assign a user to a system group, navigate to menu:Users[User List], click the username to edit, and go to the `+System Groups+` tab.
...
To manage external groups, navigate to menu:Users[System Group Configuration], and go to the `+External Authentication+` tab.
...
To see the individual clients a user can administer, navigate to menu:Users[User List], click the username to edit, and go to the `+Systems+` tab.
...
To change the default language, open the `+/etc/rhn/rhn.conf+` file and add or edit this line:
...
To change the default theme, open the `+/etc/rhn/rhn.conf+` file and add or edit this line:
