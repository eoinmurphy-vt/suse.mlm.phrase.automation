...
[NOTE]
====
An enhanced SCAP auditing experience is available as a beta feature.
It provides centralized content management, reusable scan policies, recurring scans, and built-in remediation from the {webui}.
To get started, see xref:administration:openscap-enhanced-beta.adoc[Enhanced SCAP Auditing (Beta)].
====
...
Ensure you have at least 2{nbsp}GB of RAM available on each client to be scanned (especially for {redhat}-based and other modern clients).
====
...
| SUSE Liberty footnote:[SUSE Liberty Linux includes a compatible `+scap-security-guide+` package. However, the version shipped with the client tools is not compatible, so it should not be used for SCAP content.] | 7, 8, 9, 10 | `+openscap-utils+` | `+scap-security-guide+`
...
RHEL 7 and compatible systems provide a `+scap-security-guide+` package, which contains outdated contents.
You are advised to use the `+scap-security-guide-redhat+` package you will find in the {productname} Client Tools.
...
You can do this in the same way as you move any other file, using physical storage media, or across a network with Salt (for example, https://docs.saltproject.io/en/latest/ref/cli/salt-cp.html[salt-cp] or the https://docs.saltproject.io/en/latest/ref/file_server/index.html[Salt File Server]), `+ftp+` or `+scp+`.
...
. Navigate to the `+Audit+` tab, and the `+Schedule+` subtab.

. In the `+Path to XCCDF Document+` field, enter the parameters for the SCAP template and profile you want to use on the client.
  For example:
...
[NOTE]
====
If you use `+--fetch-remote-resources+` parameter a lot of RAM is required.
In addition, you may need to increase the value of `+file_recv_max_size+`.
====
...
* `+<path_to_xccdf_file.xml>+` is the path to the content file location on the client.
  For example, `+/usr/share/xml/scap/ssg/content/ssg-sle15-ds.xml+`.

* `+<profile_name>+` is an additional argument for the `+oscap+` command.
...
Navigate to menu:Audit[OpenSCAP > All Scans] for a table of results.
...
Only SCAP scans that have passed the configured retention period can be deleted.
You can configure the retention period in the `+Organization Configuration+` section.

.Procedure: Configuring the SCAP results retention period
[role=procedure]
_____

. Navigate to menu:Home[My Organization > Configuration].

. Under `+SCAP+`, check `+Allow Deletion of SCAP Results+`.

. Enter a number of days in `+Allow Deletion After+` (default is 90).
...
. Navigate to menu:Audit[OpenSCAP > All Scans].
...
[TIP]
====
If you have the beta SCAP features enabled, you can apply remediation directly from scan results in the {webui}, including custom per-rule remediation scripts.
See xref:administration:openscap-enhanced-beta.adoc#remediation-enhanced[Enhanced SCAP Auditing - Remediation].
====
...
Install the `+scap-security-guide+` package on all your target systems.
...
. From the `+System Set Manager+`, under menu:Misc[Remote Command], write a Bash script such as:
+
