...
. Rescan the device to be resized.
  For example, to rescan `+/dev/sda+`, run the following command as root:
...
. Resize the root partition using the comamnd `+growpart+`. 
  In this example, we resize partition 3 on `+/dev/sda+`:
...
. Resize the BTRFS filesystem. 
  Be sure to use `+/.snapshots+` instead of `+/+` to avoid the read-only error:
