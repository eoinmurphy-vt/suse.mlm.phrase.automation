...
All CA certificates in the certificate chain must mark `+X509v3 Basic Constraints+` as `+critical+` and include `+CA:TRUE+`.
Strict certificate validators, including Python 3.13 and later, reject CA certificates that do not meet this requirement.
