...
. Pull the desired images, then save the images as a `+tar+` archive. 
  For example:
...
To upgrade {productname} Server, apply available host updates with `+transactional-update+`, reboot if updates were applied, then run `+mgradm upgrade podman+`.
