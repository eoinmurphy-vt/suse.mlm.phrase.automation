...
| OS:                 | Linux
| Version:            | {productname}-Proxy.x86_64-5.2.*.qcow2
| Memory:             | Minimum *)
| CPU's:              | Minimum *)
...
. Create a new virtual machine using the downloaded {minimal} KVM image and select `+Import existing disk image+`.
. Configure RAM and number of CPUs with minimum values. *)
. Name your KVM machine and select the `+Customize configuration before install+` check box.
