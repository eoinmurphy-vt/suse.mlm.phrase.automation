...
[NOTE]
====
If you want to deploy {productname} Proxy on Kubernetes, see xref:specialized-guides:kubernetes-guide/proxy-kubernetes-deployment.adoc[].
====

[IMPORTANT]
====
{sl-micro} is only supported as regular minion (`+default+` contact method) for the time being.
...
. Bootstrap the proxy as a client with the `+default+` connection method.
...
. In the `+Product Description+` field select the {sl-micro} {microversion} or {sles} {bci-mlm} checkbox then use the drop-down to select the `+{productname} Proxy Extension 5.2 x86_64+` extension.
...
* For more information, see `+mgr-storage-proxy --help+`.
...
Five {productname} Proxy containers should be present and should be part of the `+proxy-pod+` container pod:
