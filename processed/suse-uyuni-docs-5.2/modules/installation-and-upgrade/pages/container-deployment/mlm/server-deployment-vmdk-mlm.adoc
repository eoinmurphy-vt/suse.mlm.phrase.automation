...
. Download {productname} Server `+.vmdk+` file then transfer a copy to your VMware storage.
...
. Add an additional `+Hard Disk 2+` of 500 GB (or more).
...
. This step is optional.
  However, if custom persistent storage is required for your infrastructure, use the `+mgr-storage-server+` tool.
