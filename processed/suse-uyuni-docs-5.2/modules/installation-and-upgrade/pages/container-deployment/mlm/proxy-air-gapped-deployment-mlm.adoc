...
To upgrade {productname} Proxy, upgrade all packages on the host system, then run `+mgrpxy upgrade podman+`.
For air-gapped installations, upgrade the container RPM packages before running `+mgrpxy upgrade podman+`.
