...
[NOTE]
====
If you want to deploy {productname} on Kubernetes, see xref:specialized-guides:kubernetes-guide/server-kubernetes-deployment.adoc[].
====
...
Optional::
You can provide custom storage for the volumes by mounting disks on the expected volume path inside it such as `+/var/lib/containers/storage/volumes/var-spacewalk+`.
...
.Procedure: Deploying {productname} {productnumber} Using `+mgradm+`
. Log in as root.
...
. Enter your username (default is `+admin+`) and the password you set during the deployment process.
...
. From the `+Setup Wizard+` page select the btn:[Organization Credentials] tab.
...
. Back in the {productname} {webui} enter your `+Username+` and `+Password+`, and confirm with btn:[Save].
...
. From the `+Setup Wizard+` page select the `+SUSE Products+` tab.
...
This can take a long time as {productname} will copy channel software sources from the {suse} repositories located at {scclongform} to the local `+/var/lib/containers/storage/volumes/var-spacewalk/+` directory of your server.
