...
. Prepare a DVD or USB flash drive with the downloaded `+.iso+` image for installation.
...
. On the `+NTP Configuration+` page click btn:[Next].

. On the `+Authentication for the System+` page enter a password for the root user.
  Click btn:[Next].

. On the `+Installation Settings+` page click btn:[Install].
